...
class Channel extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(vedioCateList({ pageSize: 10, currentPage: 1, parentCateId: '' }))
  }
  ...
  _columns = [
    ...
    genPlanColumn('childCount', '子分类', {
      render: (text, record) => {
        return text ? (<Link to={`${urls.OPERATE_VEDIO_CHILD_CATE}/${record.cateId}`}>{text}</Link>) : null
      }
    }),
    genPlanColumn('tagCount', '标签数量', {
      render: (text, record) => {
        return (<Link to={urls.OPERATE_VEDIO_TAG + '/' + record.cateId}>{!text ? 0 : text}</Link>)
      }
    }),
  ...
  _handleChange = (pagination, filters, sorter) => {
    ...
    dispatch(vedioCateList(finalFilter))
  }
      ...
      <Form>
        ...
        >
          {getFieldDecorator('cateName', {
            ...
            <Input maxLength='50' placeholder='请输入影视分类名称' />
          )}
        </FormItem>
        <FormItem
          {...formItemLayout}
    ...
    if (data && !addChild) {
      this.setState({
        ...
        showModalWrapper((
          ...
          title: addChild ? '添加子分类' : (data ? '修改影视分类' : '添加影视分类'),
          closable: false
        })
      })
    } else {
