...
class Resource extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(sourceList({ pageSize: 10, currentPage: 1, channelId }))
  }

  _handleDelete = (channelId, sourceId) => {
    const { dispatch, filter, list, page } = this.props
    const length = list.length
    if (length > 1) {
      dispatch(deleteResource({ channelId, sourceId }, filter))
    } else if (length === 1) {
      dispatch(deleteResource({ channelId, sourceId }, { ...filter, currentPage: page.pageNo > 1 ? Number(page.currentPage) - 1 : 1 }))
    }
  }

  _columns = [
    genPlanColumn('sourceName', '资源名称'),
    genPlanColumn('sourceImgUrl', '图片', {
      render: (text) => {
        return text ? (<img alt='加载失败' style={{ width: 80, height: 80 }} src={text} />) : null
      }
    }),
    genPlanColumn('sort', '排序'),
    // genPlanColumn('linkUrl', '链接地址'),
    genPlanColumn('createTime', '添加时间'),
    genPlanColumn('modifyTime', '更新时间'),
    genPlanColumn('operate', '操作', {
      render: (text, record) => {
        return (
          <a onClick={() => this._handleDelete(record.channelId, record.sourceId)}>删除</a>
        )
      }
    }),
  ]
...
const mapStateToProps = (state) => {
  return {
    ...
    list: state.operate.channel.sourceList,
    filter: state.operate.channel.sourceFilter,
    page: state.operate.channel.sourcePage,
  }
}
