...
class ChannelChild extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(channelChildList({ cateId: this.state.cateId, currentPage: 1, pageSize: 10 }))
  }

  _handleDelete = (channelId) => {
    const { dispatch, filter, list, page } = this.props
    const length = list.length
    if (length > 1) {
      dispatch(deleteChannelChild({ channelId }, filter))
    } else if (length === 1) {
      dispatch(deleteChannelChild({ channelId }, { ...filter, currentPage: page.pageNo > 1 ? Number(page.currentPage) - 1 : 1 }))
    }
  }

  _handleConfirm = (channel) => {
    const { dispatch, filter } = this.props
    dispatch(auditConfirm({ serviceId: channel.channelId, status: 1, type: 4, snapShot: JSON.stringify(channel) }, () => dispatch(channelChildList(filter))))
  }

  _handleEnable = (channelId, isEnable) => {
    const { dispatch, filter } = this.props
    !isEnable
     ? dispatch(cancelEnableChannelChild({ channelId }, filter))
     : dispatch(enableChannelChild({ channelId }, filter))
  }

  _handleRecommend = (channelId, isRecommend) => {
    const { dispatch, filter } = this.props
    isRecommend ? dispatch(setRecommended({ channelId }, filter)) : dispatch(cancelRecommended({ channelId }, filter))
  }

  _sourceModalConfirm = (props) => {
    const { operaterPic } = this.state
    props.form.validateFields((err, values) => {
      if (!err) {
        ...
        dispatch(addChannelResource({
          ...
          sourceName: vedioSource.episodeCnName,
          sourceImgUrl: vedioSource.cover169Url || vedioSource.cover34Url || vedioSource.cover23Url,
          sort: values.sort,
  ...
  _columns = [
    genPlanColumn('channelName', '频道名称'),
    ...
    genPlanColumn('channelStatusName', '状态'),
    genPlanColumn('sourceCount', '资源数量', {
      render: (text, record) => {
        return text ? (<Link to={urls.OPERATE_CHANNEL_RESOURCE + '/' + record.channelId}>{text}</Link>) : null
      }
    }),
      ...
      render: (text, record) => {
        const editAndDelete = (
          <span>
            ...
            <Popconfirm
              title='确定要删除该条数据吗？'
              onConfirm={() => this._handleDelete(record.channelId)}
            >
              <a>删除</a>
            </Popconfirm>
          </span>
        )
        switch (record.channelStatus + '') {
          case '0':
            return (
              <span>
                {editAndDelete}
          ...
          case '2':
            return (
              <span>
                {editAndDelete}
                <Divider type='vertical' />
                <a onClick={() => this._handleSuggestion(record.channelId)}>查看原因</a>
              </span>
            )
          case '4':
            return (
              <span>
                <Popconfirm
                  placement='topRight'
                  title='确定要禁用吗？'
                  onConfirm={() => this._handleEnable(record.channelId, false)}
                >
                  <a>禁用</a>
                </Popconfirm>
                <Divider type='vertical' />
                <Popconfirm
                  ...
                  onConfirm={() => this._handleRecommend(record.channelId, !record.isRecommended)}
                >
          ...
          case '3':
            return (
              <span>
                {editAndDelete}
                <Divider type='vertical' />
                <a onClick={() => this._handleEnable(record.channelId, true)}>启用</a>
              </span>
            )
          default:
            return null
  ...
  _modalConfirm = (data, props) => {
    ...
    props.form.validateFields((err, values) => {
      if (!err) {
        const { logoImages, cateId } = this.state
        const requestBean = {
      ...
      <Form>
        ...
        >
          {getFieldDecorator('channelName', {
            ...
            initialValue: data && data.channelName ? data.channelName : undefined,
          })(
...
const mapStateToProps = (state) => {
  return {
    ...
    list: state.operate.channel.channelChildList,
    filter: state.operate.channel.channelChildFilter,
    page: state.operate.channel.channelChildPage,
