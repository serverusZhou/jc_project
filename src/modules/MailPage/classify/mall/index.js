...
class classifyMall extends Component {
  constructor (props) {
    super(props)
    this.state = {
      visible: false,
  ...
  _getParameter = (currentPage = 1, pageSize = 10) => {
    return { ...this.state.filterData, currentPage, pageSize, businessType: '1' }
  }
  // 发起列表查询的 ACTION
  _handleAction = (page = 1) => {
    ...
    dispatch(actions.getClassify({ businessType: '1' }))
  }
  // 点击查询按钮时，根据参数获取表格数据
  ...
  _handleOk = () => {
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        const arg = { businessType: '1', parentId: parentType === '0' ? '0' : parentId, ...value }
        if (type === '0') {
