...
//  审核类型：1=广告 2=布局 3=媒资
const auditType = OperateAuditType.Interphase
const statusColor = {
  wait: 'orange',
  pass: 'green-inverse',
  fail: 'red-inverse'
...
class InterphaseDetail extends React.Component {
  ...
  componentWillMount() {
    ...
    this.props.dispatch(
      getInterphaseAuditLog({ serviceId: serviceId, type: auditType })
    )
  }
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      dispatch(postInterphaseAudit(fieldsValue))
    })
  }
  ...
  render() {
    ...
    return (
      <Card title={this.getCardTitle()}>
        <Form>
          <FormItem {...formItemLayout} label='界面位置'>
            {detail.channelPosition}
          </FormItem>
          <FormItem {...formItemLayout} label='界面图'>
            {detail.channelLogoUrl &&
              <img
                ...
                src={detail.channelLogoUrl}
                alt='界面图'
                onClick={() => this.handlePreview(detail.channelLogoUrl)}
              />}
          </FormItem>
...
const mapStateToProps = state => {
  return {
    detail: state.resource.audit.interphaseAuditDetail,
    auditLogs: state.resource.audit.interphaseAuditLogs,
