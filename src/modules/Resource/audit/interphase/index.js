...
import { getInterphaseList } from '../reduck'
import { RESOURCE_AUDIT_INTERPHASE_DETAIL } from 'Global/urls'
...
class Interphase extends Component {
  ...
  _handleSearch = searchData => {
    ...
    searchData.type = OperateAuditType.Interphase
    const finalFilter = Object.assign({}, filter, searchData, { currentPage: 1, pageSize: page.pageSize })
    this.props.dispatch(getInterphaseList(finalFilter))
  }
  ...
  _handleChange = (pagination, filters, sorter) => {
    ...
    const finalFilter = { ...filter, currentPage: page.pageSize !== pageSize ? 1 : current, pageSize, type: OperateAuditType.Interphase }
    dispatch(getInterphaseList(finalFilter))
  }

  // 生成表格展示字段
  _getColumns = (tab) => {
    const columns = [
      genPlanColumn('channelPosition', '界面位置'),
      // genPlanColumn('channelName', '界面名称'),
      {
        key: 'channelLogoUrl',
        title: '界面图',
        align: 'center',
        className: styles['table-image-column'],
        dataIndex: 'channelLogoUrl',
        render: (text, record, index) =>
          (
            record.channelLogoUrl && <img src={record.channelLogoUrl} className={styles['interphase-preview']} />
          )
      }
    ]
    ...
    const operationColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_INTERPHASE_DETAIL}/${record.serviceId}`}>
            查看
          </Link>)
      }
    ]
    ...
    const viewColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_INTERPHASE_DETAIL}/${record.serviceId},${record.auditId}`}>
            查看
          </Link>)
      }
    ]
...
const mapStateToProps = state => {
  return {
    list: state.resource.audit.interphaseList,
    page: state.resource.audit.interphasePage,
    filter: state.resource.audit.interphaseFilter,
    showListSpin: state.common.showListSpin,
    positions: state.resource.audit.interphasePositions
  }
}
