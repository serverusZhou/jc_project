...
import { getChannelCateList } from '../../reduck'
import { RESOURCE_AUDIT_LIVE_CATE_DETAIL } from 'Global/urls'
...
class ChannelCate extends Component {
  ...
  _handleSearch = searchData => {
    ...
    searchData.type = OperateAuditType.LiveChannelCate
    ...
    this.props.dispatch(getChannelCateList(finalFilter))
  }
  ...
  _handleChange = (pagination, filters, sorter) => {
    ...
    const finalFilter = { ...filter, currentPage: page.pageSize !== pageSize ? 1 : current, pageSize, type: OperateAuditType.LiveChannelCate }
    dispatch(getChannelCateList(finalFilter))
  }
    ...
    const operationColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_LIVE_CATE_DETAIL}/${record.serviceId}`}>
            查看
          </Link>)
      }
    ]
    ...
    const viewColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_LIVE_CATE_DETAIL}/${record.serviceId},${record.auditId}`}>
            查看
          </Link>)
      }
    ]
...
const mapStateToProps = state => {
  return {
    list: state.resource.audit.livecateList,
    page: state.resource.audit.livecatePage,
    filter: state.resource.audit.livecateFilter,
...
export default connect(mapStateToProps, mapDispatchToProps)(ChannelCate)
