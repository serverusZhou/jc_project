...
import { getChannelList } from '../../reduck'
import { RESOURCE_AUDIT_LIVE_CHANNEL_DETAIL } from 'Global/urls'
...
class Channel extends Component {
  ...
  componentWillMount() {
    this._handleSearch({
      ...
      channelStatus: SearchAuditStatusKeyMap.WAIT.value
    })
  }
  ...
  _handleSearch = searchData => {
    ...
    searchData.type = OperateAuditType.LiveChannel
    searchData.channelStatus = SearchAuditStatusKeyMap.WAIT.value
    ...
    this.props.dispatch(getChannelList(finalFilter))
  }
  ...
  _handleChange = (pagination, filters, sorter) => {
    ...
    const finalFilter = { ...filter, currentPage: page.pageSize !== pageSize ? 1 : current, pageSize, type: OperateAuditType.LiveChannel }
    dispatch(getChannelList(finalFilter))
  }

  // 生成表格展示字段
  _getColumns = (tab) => {
    const columns = [
      genPlanColumn('channelName', '直播频道'),
      // genPlanColumn('cateId', '分类ID'),
    ...
    const operationColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_LIVE_CHANNEL_DETAIL}/${record.serviceId}`}>
            查看
          </Link>)
      }
    ]
    ...
    const viewColumns = [
      {
        ...
        align: 'center',
        render: (text, record, index) =>
          (<Link to={`${RESOURCE_AUDIT_LIVE_CHANNEL_DETAIL}/${record.serviceId},${record.auditId}`}>
            查看
          </Link>)
      }
    ]
...
const mapStateToProps = state => {
  return {
    list: state.resource.audit.livechannelList,
    page: state.resource.audit.livechannelPage,
    filter: state.resource.audit.livechannelFilter,
