...
//  审核类型：1=广告 2=布局 3=媒资
const auditType = OperateAuditType.OTTCHANNEL
const statusColor = {
  wait: 'orange',
  pass: 'green-inverse',
  fail: 'red-inverse'
...
class InterphaseDetail extends React.Component {
  ...
  componentWillMount() {
    ...
    const params1 = this.props.match.params.id.split('@')
    const serviceId = params[0].split('@')[0]
    const auditId = params[1]
    const id = params1[1]
    this.setState({ serviceId, auditId, id })
    // 通过id获取详情
    this.props.dispatch(
      getOTTChannelDetail({
        serviceId,
...
const mapStateToProps = state => {
  return {
    detail: state.resource.audit.channelAuditDetail || [],
    auditLogs: state.resource.audit.channeluditLogs,
