...
class AddAudit extends Component {
  ...
  _handleSubmit(e) {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        const arg = {
          logoUrl: logoUrl && logoUrl[0] && logoUrl[0].url,
  ...
  render() {
    ...
    const { logoUrl, downloadUrl } = this.state
                ...
                {getFieldDecorator('logoUrl', {
                  ...
                  <div>
                    <OrderUpload
                      ...
                      onChange={(e) => this._handleCoverChange(e, 'logoUrl')}
                      aliToken={aliToken}
                    ...
                    >
                      {logoUrl && logoUrl.length > 0 ? <img src={logoUrl && logoUrl[logoUrl.length - 1] && logoUrl[logoUrl.length - 1].url} style={{ width: '100%', height: '100%' }} /> : uploadButton}
                    </OrderUpload>
                  </div>
                )}
              </FormItem>
