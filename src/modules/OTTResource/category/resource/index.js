...
class Resource extends Component {
  ...
  _handleDelete = (cateId, sourceId) => {
    const { dispatch, filter, list, page } = this.props
    const length = list.length
    if (length > 1) {
      dispatch(deleteResource({ cateId, sourceId }, filter))
    } else if (length === 1) {
      dispatch(deleteResource({ cateId, sourceId }, { ...filter, currentPage: page.pageNo > 1 ? Number(page.currentPage) - 1 : 1 }))
    }
  }
  ...
  _columns = [
    ...
    genPlanColumn('operate', '操作', {
      render: (text, record) => {
        return (
          <a onClick={() => this._handleDelete(record.cateId, record.sourceId)}>删除</a>
        )
      }
    }),
  ]
...
const mapStateToProps = (state) => {
  return {
    ...
    list: state.oTTResource.oTTCategory.sourceList,
    filter: state.oTTResource.oTTCategory.sourceFilter,
    page: state.oTTResource.oTTCategory.sourcePage,
  }
}
